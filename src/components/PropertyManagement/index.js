  ...
  const handleGetLocation = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        setSelectedLocality(`Lat: ${latitude}, Lng: ${longitude}`);
      }, (error) => {
        console.error(error);
  ...
  const filterProperties = (properties) => {
    return properties.filter((property) => {
      ...
      const matchesLocality = selectedLocality
        ? property.Locality.toLowerCase().includes(selectedLocality.toLowerCase())
        : true;

      return matchesBudget && matchesLocality;
    });
  };

  const renderProperties = () => {
    if (selectedCity && propertiesByCity[selectedCity]) {
      const filteredProperties = filterProperties(propertiesByCity[selectedCity]);
      return filteredProperties.map((property) => (
        <div key={property.id} className="property-card-prop">
          <img src={property.propertyPhoto} alt={property.propertyName} className="property-photo-prop" />
    ...
    return <p>Select a city to see properties</p>;
  };
  ...
  return (
    <>
      ...
      <div className="property-management-container">
        ...
        <div className="properties-list-prop">
          {renderProperties()}
        </div>
      </div>
    </>
